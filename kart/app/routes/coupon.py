...
def check_coupon():
    ...
    elif "\"code\":1107" in r.text:
        return "{\"result\":\"false\", \"message\":\"쿠폰 사용 기간이 아닙니다.\"}"
    ...
    elif "\"code\":94202" in r.text:
        return "{\"result\":\"false\", \"message\":\"쿠폰 길이가 정확하지 않습니다.\"}"
...
@coupon_api.route("/submit_all", methods=["POST"])
def submit_all():
    coupon = request.form["coupon"]
    ...
    with db_conn() as conn:
        cur = conn.cursor()
        cur.execute("SELECT userid FROM coupon_users")
        res = cur.fetchall()
